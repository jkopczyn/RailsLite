require 'webrick'
require 'phaseHigher/router'
require 'phaseHigher/controller_base'

describe PhaseHigher::Route do
  ...
  describe "#matches?" do
    it "matches simple regular expression" do
      index_route = PhaseHigher::Route.new(Regexp.new("^/users$"), :get, "x", :x)
    ...
    it "matches regular expression with capture" do
      index_route = PhaseHigher::Route.new(Regexp.new("^/users/(?<id>\\d+)$"), :get, "x", :x)
      allow(req).to receive(:path).and_return("/users/1")
      index_route.matches?(req).should be true
    end

    it "correctly doesn't matche regular expression with capture" do
      index_route = PhaseHigher::Route.new(Regexp.new("^/users/(?<id>\\d+)$"), :get, "UsersController", :index)
  ...
  describe "#run" do
    ...
    it "instantiates controller and invokes action" do
      ...
      index_route = PhaseHigher::Route.new(Regexp.new("^/users$"), :get, dummy_controller_class, :index)
      index_route.run(req, res)
    end
  end
end

describe PhaseHigher::Router do
  let(:req) { WEBrick::HTTPRequest.new(Logger: nil) }
  let(:res) { WEBrick::HTTPResponse.new(HTTPVersion: '1.0') }
  ...
  describe "http method (get, put, post, delete)" do
    ...
    it "adds a route when an http method method is called" do
      router = PhaseHigher::Router.new
      router.get Regexp.new("^/users$"), PhaseHigher::ControllerBase, :index
