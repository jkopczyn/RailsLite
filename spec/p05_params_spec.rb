require 'webrick'
require 'phaseHigher/params'
require 'phaseHigher/controller_base'
require 'byebug'

describe PhaseHigher::Params do
  before(:all) do
  ...
  it "handles an empty request" do
    expect { PhaseHigher::Params.new(req) }.to_not raise_error
  end

  context "query string" do
    it "handles single key and value" do
      req.query_string = "key=val"
      params = PhaseHigher::Params.new(req)
  ...
  context "route params" do
    it "handles route params" do
      params = PhaseHigher::Params.new(req, {"id" => 5, "user_id" => 22})
      params["id"].should == 5
  ...
  #       expect { params.require("key2") }.to raise_error(PhaseHigher::Params::AttributeNotFoundError)
  #     end
  #   end
  #
